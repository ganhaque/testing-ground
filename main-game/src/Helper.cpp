...
Texture2D Helper::loadTexture(std::string filePath) {
  std::string fullFilePath = "./assets/" + filePath;
  return LoadTexture(fullFilePath.c_str());
}

std::string Helper::parseGameObjectType(const std::string& input) {
...
void Helper::parseString(const std::string& input, int& intValue, std::string& stringValue) {
...
void Helper::parseCoordinate(const std::string& input, int& firstInt, int& secondInt) {
  // Find the position of the hyphen
  size_t hyphenPos = input.find('-');
